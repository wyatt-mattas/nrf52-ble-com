...
struct LedControlService {
    control_handle: u16,
}

impl LedControlService {
    pub fn new(sd: &mut Softdevice) -> Result<Self, RegisterError> {
        let mut service_builder = ServiceBuilder::new(sd, LED_SERVICE_UUID)?;

        let attr = Attribute::new(&[0u8]).security(SecurityMode::Open);
        let metadata = Metadata::new(Properties::new().read().write());
        let characteristic_builder = service_builder.add_characteristic(LED_CONTROL_CHAR_UUID, attr, metadata)?;
        let characteristic_handles = characteristic_builder.build();

        Ok(LedControlService {
            control_handle: characteristic_handles.value_handle,
        })
    }

    pub fn on_write(&self, handle: u16, data: &[u8]) {
        if handle == self.control_handle {
            // Assume `data` is [0] or [1] to turn off/on the LED respectively.
            // Implement the logic to toggle the LED based on `data[0]` value.
            if let Some(led_state) = data.first() {
                match led_state {
                    0 => { /* Turn LED off */ },
                    1 => { /* Turn LED on */ },
                    _ => defmt::warn!("Invalid LED state received"),
                }
            }
        }
    }
